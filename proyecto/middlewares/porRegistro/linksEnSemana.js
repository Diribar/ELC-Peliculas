...
module.exports = async (req, res, next) => {
	...
	if (!sigProd) return res.redirect("/miscelaneas/inactivar-captura/" + entidad + "/?id=" + id + "&origen=TE");
	...
	if (entidad != sigProd.entidad || id != sigProd.id) {
		...
		if (!sigProd || entidad != sigProd.entidad || id != sigProd.id)
			return res.redirect("/miscelaneas/inactivar-captura/" + entidad + "/?id=" + id + "&origen=TE");
	}
