...
module.exports = {
	...
	actualizaSolapam: async () => {
		...
		let fechasDelAnoFN = BD_genericas.obtieneTodos("fechasDelAno");
		[epocasDelAno, fechasDelAnoFN] = await Promise.all([epocasDelAno, fechasDelAnoFN]);
		...
		for (let epocaDelAno of epocasDelAno) {
			...
			for (let i = 0; i < epocaDelAno.diasDeDuracion; i++) {
				...
				const fechaDelAno = fechasDelAnoFN[indice];
				fechaDelAno.epocaDelAno_id == 1
					? (fechasDelAnoFN[indice] = {...fechaDelAno, epocaDelAno_id: epocaDelAno.id}) // en caso positivo le asigna el id de la epocaDelAno
			...
			const IDs = fechasDelAnoFN.filter((n) => n.epocaDelAno_id == epocaDelAno.id).map((n) => n.id); // obtiene los IDs de las fechas de la epocaDelAno
