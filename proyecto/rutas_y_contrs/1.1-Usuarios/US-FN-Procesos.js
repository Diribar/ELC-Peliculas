...
module.exports = {
	...
	eliminaDuplicados: async (usuario_id) => {
		...
		const registros = await baseDeDatos.obtieneTodosPorCondicion("diarioNavegs", {usuario_id});

		// Elimina los duplicados
		for (let i = registros.length - 1; i > 0; i--)
			if (registros[i].fecha == registros[i - 1].fecha) baseDeDatos.eliminaPorId("diarioNavegs", registros[i].id);
